...
![并行和并发](https://img.java-family.cn/20220907153132.png)

就好像我们去食堂打饭，并行就是我们在多个窗口排队，几个阿姨同时打菜；并发就是我们挤在一个窗口，阿姨给这个打一勺，又手忙脚乱地给那个打一勺。

![](https://img.java-family.cn/20230514181327image-20230514181327100.png)
...
![](https://img.java-family.cn/20220907153141.png)
...
![](https://img.java-family.cn/20220907153150.png)

**为什么我们不能直接调用run()方法？**也很清楚， 如果直接调用Thread的run()方法，那么run方法还是运行在主线程中，相当于顺序执行，就起不到多线程的效果。

## 4.线程有哪些常用的调度方法？

![](https://img.java-family.cn/20220907153155.png)
...
![](D:\BlogImage\知识星球：面试题\并发编程\7.png)

线程在自身的生命周期中， 并不是固定地处于某个状态，而是随着代码的执行在不同的状态之间进行切换，Java线程状态变化如图示：

![Java线程状态变化](https://img.java-family.cn/20220907153200.png)

## 6.什么是线程上下文切换？

使用多线程的目的是为了充分利用CPU，但是我们知道，并发其实是一个CPU来应付多个线程。

![](https://img.java-family.cn/20220907153210.png)

为了让用户感觉多个线程是在同时执行的， CPU 资源的分配采用了时间片轮转也就是给每个线程分配一个时间片，线程在时间片内占用 CPU 执行任务。当线程使用完时间片后，就会处于就绪状态并让出 CPU 让其他线程占用，这就是上下文切换。

![上下文切换时机](https://img.java-family.cn/20220907153216.png)
...
![线程间通信方式](https://img.java-family.cn/20220907153222.png)
...
![ThreadLocal线程副本](https://img.java-family.cn/20220907153226.png)
...
![ThreadLoca存放用户上下文](https://img.java-family.cn/20220907153232.png)
...
![ThreadLoca结构图](https://img.java-family.cn/20220907153236.png)
...
![内存分配](https://img.java-family.cn/20220907153239.png)
...
![ThreadLocalMap结构示意图](https://img.java-family.cn/20220907153244.png)
...
![ThreadLocalMap解决冲突](https://img.java-family.cn/20220907153248.png)
...
![ThreadLocalMap扩容](https://img.java-family.cn/20220907153252.png)

具体代码：

![ThreadLocalMap resize](https://img.java-family.cn/20220907153255.png)
...
![Java内存模型](https://img.java-family.cn/20220907153302.png)

本地内存是JMM的 一个抽象概念，并不真实存在。它其实涵盖了缓存、写缓冲区、寄存器以及其他的硬件和编译器优化。

![实际线程工作模型](https://img.java-family.cn/20220907153305.png)
...
![多级指令重排](https://img.java-family.cn/20220907153309.png)
...
![双重校验单例模式异常情形](https://img.java-family.cn/20220907153312.png)
...
![happens-before六大规则](https://img.java-family.cn/20220907153315.png)
...
![](https://img.java-family.cn/20220907153319.png)

A和C之间存在数据依赖关系，同时B和C之间也存在数据依赖关系。因此在最终执行的指令序列中，C不能被重排序到A和B的前面（C排到A和B的前面，程序的结果将会被改变）。但A和B之间没有数据依赖关系，编译器和处理器可以重排序A和B之间的执行顺序。

所以最终，程序可能会有两种执行顺序：

![两种执行结果](https://img.java-family.cn/20220907153322.png)
...
![volatile内存可见性](https://img.java-family.cn/20220907153325.png)

**volatile怎么保证有序性的呢？**

重排序可以分为编译器重排序和处理器重排序，valatile保证有序性，就是通过分别限制这两种类型的重排序。

![volatile重排序规则表](https://img.java-family.cn/20220907153329.png)
...
![volatile写插入内存屏障后生成的指令序列示意图](https://img.java-family.cn/20220907153333.png)

![volatile写插入内存屏障后生成的指令序列示意图](https://img.java-family.cn/20220907153338.png)
...
![monitorenter和monitorexit](https://img.java-family.cn/20220907153345.png)

synchronized修饰同步方法时，JVM采用`ACC_SYNCHRONIZED`标记符来实现同步，这个标识指明了该方法是一个同步方法。

同样可以写段代码反编译看一下。

![synchronized修饰同步方法](https://img.java-family.cn/20220907153348.png)
...
![](https://img.java-family.cn/20220907153353.png)
...
![Montior机制](https://img.java-family.cn/20220907153356.png)
...
![Mark Word变化](https://img.java-family.cn/20220907153400.png)
...
![锁升级方向](https://img.java-family.cn/20220907153403.png)
...
![锁升级简略过程](https://img.java-family.cn/20220907153420.png)

完整的升级过程：

![synchronized 锁升级过程](https://img.java-family.cn/20220907153423.png)
...
![synchronized和ReentrantLock的区别](https://img.java-family.cn/20220907153511.png)
...
![ReentrantLock 非公平锁加锁流程简图](https://img.java-family.cn/20220907153532.png)
...
![公平锁tryAcquire](https://img.java-family.cn/20220907153537.png)
...
![CAS三大问题](https://img.java-family.cn/20220907153541.png)
...
![Java保证原子性方法](https://img.java-family.cn/20220907153544.png)
...
![原子操作类](https://img.java-family.cn/20220907153548.png)
...
![死锁示意图](https://img.java-family.cn/20220907153552.png)

那么为什么会产生死锁呢？死锁的产生必须具备以下四个条件：

![死锁产生必备四条件](https://img.java-family.cn/20220907153555.png)
...
![线程死锁检测](https://img.java-family.cn/20220907153558.png)
...
![CountDownLatch模仿这个场景](https://img.java-family.cn/20220907153602.png)
...
![王者荣耀-](https://img.java-family.cn/20220907153606.png)
...
![](D:\BlogImage\知识星球：面试题\并发编程\58.png)
...
![](https://img.java-family.cn/20220907153635.png)
...
![](https://img.java-family.cn/20220907153639.png)
...
![](https://img.java-family.cn/20220907153643.png)

主要代码如下：

![](https://img.java-family.cn/20220907153646.png)
...
![直接办理](https://img.java-family.cn/20220907153656.png)

2、陈哥发现没有空闲的窗口，就在排队区排队等。

![排队等待](https://img.java-family.cn/20220907153701.png)

3、陈哥发现没有空闲的窗口，等待区也满了，蚌埠住了，经理一看，就让休息的小姐姐赶紧回来上班，等待区号靠前的赶紧去新窗口办，陈哥去排队区排队。小姐姐比较辛苦，假如一段时间发现他们可以不用接着营业，经理就让她们接着休息。

![排队区满](https://img.java-family.cn/20220907153704.png)

4、陈哥一看，六个窗口都满了，等待区也没位置了。陈哥急了，要闹，经理赶紧出来了，经理该怎么办呢？

![等待区，排队区都满](https://img.java-family.cn/20220907153707.png)
...
![线程池执行流程](https://img.java-family.cn/20220907153712.png)
...
![线程池参数](https://img.java-family.cn/20220907153716.png)
...
![四种策略](https://img.java-family.cn/20220907153720.png)
...
![线程池常用阻塞队列](https://img.java-family.cn/20220907153723.png)
...
![](https://img.java-family.cn/20220907153728.png)
...
![四大线程池](https://img.java-family.cn/20220907153731.png)
...
![SingleThreadExecutor运行流程](https://img.java-family.cn/20220907153736.png)
...
![FixedThreadPool](https://img.java-family.cn/20220907153739.png)
...
![CachedThreadPool执行流程](https://img.java-family.cn/20220907153743.png)
...
![ScheduledThreadPool执行流程](https://img.java-family.cn/20220907153748.png)
...
![ScheduledThreadPoolExecutor执行流程](https://img.java-family.cn/20220907153751.png)
...
![线程池异常处理](https://img.java-family.cn/20220907153756.png)
...
![线程池状态切换图](https://img.java-family.cn/20220907153759.png)
...
![JDK 线程池参数设置接口来源参考[7]](https://img.java-family.cn/20220907153803.png)

这里主要有两个思路：

![动态修改线程池参数](https://img.java-family.cn/20220907153806.png)
...
![线程池评估方案](https://img.java-family.cn/20220907153809.png)
...
![线程池调优](https://img.java-family.cn/20220907153812.png)
...
![线程池主要实现流程](https://img.java-family.cn/20220907153815.png)
...
![自定义线程池](https://img.java-family.cn/20220907153818.png)
...
![Fork/Join分治算法](https://img.java-family.cn/20220907153824.png)
...
![工作窃取](https://img.java-family.cn/20220907153827.png)
